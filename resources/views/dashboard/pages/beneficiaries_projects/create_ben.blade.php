...
@section('content')
 ...
 <button  class="btn btn-outline-secondary" onclick="history.back()">اغلاق</button>
            <form method="post"
                ...
                <input type="hidden" value="{{$branchCount}}" name="branchCount">
...
@section('js')
    ...
    <script>
        ...
        let project_table = $('.project-list-table').DataTable({
            ...
            ajax: {
                url: '{{ route('beneficiareis.createBen') }}',
                data: {'project_id': {{ $project_id }}, 'branchCount': {{$branchCount}} }
            },
            columns: [
    ...
    <script>
        ...
        $(document).on('click', '#submit_all_beneficiareis', function (e) {
            ...
            $.ajax({
                type: 'post',
                url: "{{ route('projects.submit.all2.beneficiaries') }}",
                data: submitAll,
                processData: false,
                contentType: false,
                cache: false,
